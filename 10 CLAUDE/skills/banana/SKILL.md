...
```bash
& "$HOME/.venvs/banana/Scripts/python.exe" "${CLAUDE_SKILL_DIR}/scripts/presets.py" list
```
...
| Error | Resolution |
|-------|-----------|
| Python venv not found | Create the banana venv: `uv venv "$HOME/.venvs/banana" --python 3.12` |
...
| MCP unavailable | Fall back to direct API via `uv` venv: `& "$HOME/.venvs/banana/Scripts/python.exe" "10 CLAUDE/skills/banana/scripts/generate.py" --prompt "..." --aspect-ratio "16:9"` or `& "$HOME/.venvs/banana/Scripts/python.exe" "10 CLAUDE/skills/banana/scripts/edit.py" --image PATH --prompt "..."`. These call the Gemini REST API directly with no MCP dependency. |
...
```bash
& "$HOME/.venvs/banana/Scripts/python.exe" "${CLAUDE_SKILL_DIR}/scripts/cost_tracker.py" log --model MODEL --resolution RES --prompt "brief description"
```
